...
// Comercios referidos (restaurantes, farmácias, mercados, etc.) que la IA
// recomienda al huésped. Solo 2 planes, como se definió: gratis y premium.
// La única diferencia hoy es la prioridad de aparición — el premium se
// muestra primero cuando hay varios comercios que matchean la misma consulta.
export const COMERCIO_PLAN_LIMITS = {
  gratis: { priority: 1 },
  premium: { priority: 2 },
};
...
// Mismo criterio, pero para comercios referidos, que solo tienen 2 planes.
export function defaultPlanesHabilitadosComercio() {
  return { gratis: true, premium: false };
}
...
// Los comercios siguen el mismo criterio que los técnicos: gratis siempre libre.
export function puedeDesactivarsePlan(tipoUsuario, plan) {
  if ((tipoUsuario === "tecnico" || tipoUsuario === "comercio") && plan === "gratis") return false;
  return true;
}
