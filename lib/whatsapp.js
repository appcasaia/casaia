...
export async function sendWhatsAppTemplate({ to, templateName, languageCode = "pt_BR", variables = {} }) {
  ...
  // WhatsApp rechaza parámetros de plantilla que tengan saltos de línea,
  // tabs, o más de 4 espacios seguidos (código 132018). Sanitizamos acá,
  // en un solo lugar, para que cubra cualquier variable que se mande
  // (por ejemplo el resumen del diagnóstico de la IA, que puede traer
  // saltos de línea de formato).
  function sanitizeParam(value) {
    return String(value ?? "-")
      .replace(/[\r\n\t]+/g, " ") // saltos de línea/tabs -> espacio
      .replace(/ {2,}/g, " ") // colapsar espacios múltiples a uno solo
      .trim();
  }
